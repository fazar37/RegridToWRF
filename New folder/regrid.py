...
def calculate_population_in_grid(gdf, wrf_info, Lgdf, cols, vanIf=True):
    ...
    for _, row in gdf.iterrows():
        ...
        if not vanIf: prcCorc = intersection_with_land_area / 100
        ...
        for i, j, cell in grid_polygons:
            ...
            if not land_intersection.is_empty and land_intersection.is_valid:
                ...
                for ii, col in enumerate(cols):
                    population_grid[col][i, j] += row[col] * area_weight * prcCorc
...
def saveNC(output_netcdf:str, wrf_info, values, cols):
