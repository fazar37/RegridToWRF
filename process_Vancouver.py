...
from regrid import calculate_population_in_grid, get_wrf_grid_info, saveNC
...
def population(wrf_file, shapefile, output, landshapefile, cols, vanIf=True):
    ...
    # Regriding data
    population_grid = calculate_population_in_grid(gdf, wrf_info, Lgdf, cols, vanIf=vanIf)
    # Load the WRF data,
    ds = Dataset(wrf_file)
    population_grid['lats'], population_grid['lons'] = ds['XLAT'][0,:,:], ds['XLONG'][0,:,:]
    ds.close()

    saveNC(output, wrf_info, population_grid, cols)
